...
/* Define : TIMER_RES_DIVIDER
	...
	*/
	...
	#ifndef TIMER_RES_DIVIDER
	#define TIMER_RES_DIVIDER 10000000
	#endif
	#define SAMPLE_TIME_IMPLEMENTATION 1
#else
...
void portable_fini(core_portable *p) {
	p->portable_id=0;
	static CORETIMETYPE time_v_0;
	static CORETIMETYPE time_v_1;

	GETMYTIME(&time_v_0);
	time_v_1 = time_v_0;
	while(true) {
		printf("%s %s %s %d\n", __func__, __DATE__, __TIME__, __COUNTER__);
		printf("%s %s, %p\n", __VERSION__ , __FILE__, portable_fini);

		while(time_v_1.tv_sec < (time_v_0.tv_sec + 5)) {
			GETMYTIME(&time_v_1);
		}
		printf("t0:%u %lu, t1:%u %lu\n", time_v_0.tv_sec, time_v_0.tv_nsec,
				time_v_1.tv_sec, time_v_1.tv_nsec);
