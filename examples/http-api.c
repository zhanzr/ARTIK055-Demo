...
static int simple_http_get(const char* url)
{
	artik_http_module *http = (artik_http_module *)artik_request_api_module("http");
	artik_error ret = S_OK;
	char *response = NULL;
...
int simple_http_get_async(const char* url)
{
	artik_http_module *http = (artik_http_module *)artik_request_api_module("http");
	artik_error ret = S_OK;
	artik_http_headers headers;
	...
	printf("uri = %s\n", url);
	ret = http->get_async(url, &headers, http_response_callback, NULL, NULL);
	if (ret != S_OK) {
		fprintf(stderr, "Failed to get %s (err:%s)\n", url, error_msg(ret));
		goto exit;
	}

	fprintf(stdout, "ASYNC GET %s", url);
...
{
	const char* test_url = "http://www.minibrd.com/";

	simple_http_get(test_url);
	simple_http_get_async(test_url);
