...
static int startsta_command(int argc, char *argv[]) {
  ...
  artik_wifi_module *wifi =
      (artik_wifi_module *)artik_request_api_module("wifi");
...
static int startap_command(int argc, char *argv[]) {
  ...
  err =
      wifi->start_ap(argv[3], passphrase, atoi(argv[4]),
...
static int dhcp_command(int argc, char *argv[]) {
  int ret = 0;
  artik_network_module *network =
      (artik_network_module *)artik_request_api_module("network");
  artik_error err = S_OK;
